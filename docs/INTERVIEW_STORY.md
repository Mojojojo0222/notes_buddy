...
### ArgoCD
- GitOps model: Git as single source of truth, ArgoCD pulls changes
- Architecture: API Server, Repo Server, Application Controller, Redis, Dex
- Application CRD: source (repo+path+revision), destination (cluster+namespace), syncPolicy
- Sync phases and waves: PreSync → Sync → PostSync (dependency ordering)
- Auto-sync: prune (delete removed resources), selfHeal (fix manual changes)
- App-of-apps pattern: root Application deploys child Applications
- Helm chart install via `argo/argo-cd` chart
- Drift detection: continuous comparison of Git vs cluster
- Rollback: revert Git commit, ArgoCD syncs automatically
- ArgoCD vs push-based CI/CD: security, drift detection, self-healing
...
### Phase 9: Local-First Features (Day 11)

**Goal:** Make the dashboard useful day-to-day without needing a cluster.

**Five features built in one session:**

1. **Exit Codes** — Every command now captures `$?` (exit code) as a 5th field in the log format. Displayed as a ✓ (green) or ✗ N (red) badge next to each command. Weekly summary shows total error count. Foundation for future error-tracking features.

2. **Tag Commands from UI** — Click a `+` badge on any command to add a tag/label inline. Press Enter to save, Esc to cancel. Tags persist to the database via `POST /commands/{id}/tag`.

3. **Timeline View** — Date picker with Prev/Next/Today navigation. Picking any date loads all commands for that day from `GET /commands/by-date?date=YYYY-MM-DD`. No more "today only" limitation.

4. **Weekly Summary** — New "This Week" section shows command count, most-used tool, categories touched, and error count for the last 7 days.

5. **Filter Persistence** — Active category filters survive page reload via localStorage. No need to re-select filters every time.

**Problems hit:**
- None. All 5 features were backward-compatible — old log lines without exit code still parse correctly via `split("\\|", 5)` fallback.
- DDL auto-update added the new columns without data loss.

**Key learning:** Exit codes alone unlock error tracking across the entire history. Every failed command is now findable, countable, and taggable.

---

### Phase 10: Full-Text Search (Day 12)

**Goal:** Search every command ever logged — not just the visible ones in the current view.

**What was built:**
1. **Server-side search API** — `GET /commands/search?q=...` — `@Query` with JPQL `LOWER()` + `LIKE CONCAT('%', :q, '%')` across 5 fields: command text, tag, working directory, repo name, and category.

2. **Search-as-you-type UI** — 300ms debounced input. Minimum 2 characters. Results grouped by date into day cards. Works with auto-refresh — `searchQuery` global preserves search across 15-second intervals.

3. **Search-clears-date-filter pattern** — When you search, the timeline date selection is cleared. Search is always across ALL history, not scoped to one day. This avoids confusion: "why didn't that old command show up?"

**Key decisions:**
- `@Query` with JPQL not derived method name — 5-field OR search would be unreadable as a method name
- `ILIKE` not PostgreSQL `tsvector` — simpler, sufficient for <100k rows
- Same `render()` function — search results are just sessions grouped by date by the frontend

**Problems hit:** None. The API design was straightforward (one endpoint, one query). The main challenge was UX: debounce timing, grouping results, and the search-vs-timeline interaction.

**Interview story:**
> "Full-text search across all historical commands. Server-side ILIKE query covering command text, tags, working directories, repo names, and categories. 300ms debounced search-as-you-type. Results grouped by date for context. This is the feature that makes the dashboard a searchable knowledge base — not just a view of today's activity."
...
| ArgoCD | "Helm chart install, GitOps sync, app-of-apps pattern, self-heal + prune, v3.4.5" |
